...
public class as3fService extends Service {

    protected static String BASE = Util.BASE;
    protected static String BASE_BIN = Util.BASE_BIN;
    ...
    public static final String REFRESH_STATUS_INTENT = "as3fRefresh";
    protected static final String TOGGLE_STATUS_INTENT = "as3fStatusToggle";
    ...
    private class DataUpdateReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(as3fService.TOGGLE_STATUS_INTENT)) {
                Intent intentService = new Intent(context, as3fService.class);
                ...
                else if ( as3fService.current_status == Util.STATUS_CONNECTING ||
                        as3fService.current_status == Util.STATUS_SOCKS
                        )
    ...
    protected class Wait4connection extends Thread
    {
        protected void reportConnected()
        {
            ...
            if(current_status==Util.STATUS_SOCKS) {
                bm = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(res, R.drawable.cognito),
            ...
            notification.setContentTitle("AnonymousVPN");
    ...
    private class SSHThread extends Thread
    {
        ...
        public void run() {
            ...
            while( current_status != Util.STATUS_DISCONNECT && (current_fail_attempt <
                ...
                Resources res = getResources();
                Bitmap bm = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(res,R.drawable.cognito),
                        (int) res.getDimension(android.R.dimen.notification_large_icon_height),
                        (int) res.getDimension(android.R.dimen.notification_large_icon_width), false);
                notification.setLargeIcon(bm);
                notification.setContentTitle("AnonymousVPN");
    ...
    public void onCreate() {
        ...
        intentFilter.addAction(as3fService.TOGGLE_STATUS_INTENT);
    ...
    protected void stop_as3fredirect()
    {
        boolean iptables_switch = preferences.getBoolean("iptables_switch", false);
        current_status = Util.STATUS_DISCONNECT; //Disconnect
        // Notify MainActivity about status change
